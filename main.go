...
import (
	...
	api "github.com/koala-sandbox/new-backend-service/api"
)
...
func HelloKoala(w http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		fmt.Fprintln(w, "Error reading VERSION file (new-backend-service): "+err.Error())
		return
	}
	fmt.Fprintln(w, "Hello world! service:new-backend-service version: "+string(VERSION))
}
